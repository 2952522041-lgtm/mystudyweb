...
扫描版 PDF 的识别规则是：抽样检查前几页的可提取文本量；文本基本为空时仍正常显示页面，并在用户停留到该页需要翻译时，复用 AI 答疑配置的视觉模型进行 OCR。OCR 文字按文档指纹、页码、供应商、模型和提示词版本缓存在 IndexedDB，页面图像不持久化。
...
OCR 与逐页答疑复用同一套视觉服务配置，但使用独立提示词、调用入口和缓存键。文字层少于阈值时，应用按固定质量离屏渲染当前页，请求视觉模型只做逐行转录，不允许翻译、总结、解释或解题；数学表达式要求输出 LaTeX。识别结果进入原有翻译链路，也可作为课程 `DocumentDigest` 的页面来源文字。

缓存键为 `ocr:{documentFingerprint}:{pageNumber}:{provider}:{model}:v{promptVersion}`。重新翻译会绕过当前页 OCR 与译文缓存；更换视觉配置后会重新识别。课程批量导入按页顺序处理并显示 OCR 进度，未配置视觉模型时给出可行动提示，不会把扫描件误报为损坏文件。
...
本节对应产品设计第 12 节。当前已经实现浏览器绑定本地课程文件夹的存储方式，并支持文字层提取与视觉 OCR 后生成结构化内部摘要。课程目录中的结构化文件是业务数据的唯一可信来源，IndexedDB 仅保存目录句柄、OCR/翻译缓存、任务恢复信息和服务设置。技术边界仍允许后续增加 Tauri、浏览器内部存储或云端实现。
...
interface KnowledgeProvider {
  ...
  extractConversationInsights(input: ConversationInput): Promise<LearningInsight[]>;
}
```
